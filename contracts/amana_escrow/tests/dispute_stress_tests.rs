...
impl Stress {
    ...
    fn disputed_trade(&self, amount: i128) -> u64 {
        ...
        self.client().initiate_dispute(
            &tid,
            &self.buyer,
            &SStr::from_str(&self.env, "QmStressDispute"),
        );
        tid
    }
}
...
fn test_stress_evidence_fifo_order_10_submissions() {
    ...
    for (i, submitter) in submitters.iter().enumerate() {
        s.env
            .ledger()
            .with_mut(|l| l.timestamp = 1_000 + i as u64 * 100);
...
fn test_stress_evidence_hash_immutable_after_submission() {
    ...
    s.client()
        .submit_evidence(&tid, &s.buyer, &cid_buyer, &SStr::from_str(&s.env, "d1"));
    s.client()
        .submit_evidence(&tid, &s.seller, &cid_seller, &SStr::from_str(&s.env, "d2"));

    // Submit more evidence — must not overwrite earlier entries
    s.client().submit_evidence(
        &tid,
        &s.buyer,
        &SStr::from_str(&s.env, "QmBuyerSecond"),
        &SStr::from_str(&s.env, "d3"),
    );
...
fn test_stress_evidence_order_deterministic_mixed_parties() {
    ...
    for (i, (submitter, cid_str)) in sequence.iter().enumerate() {
        s.env
            .ledger()
            .with_mut(|l| l.timestamp = 2_000 + i as u64 * 50);
        s.client().submit_evidence(
            &tid,
            submitter,
...
fn test_stress_registry_churn_interleaved_with_evidence() {
    ...
    s.client().submit_evidence(
        &tid,
        &med_a,
        &SStr::from_str(&s.env, "QmMedA1"),
        &SStr::from_str(&s.env, "d"),
    );
    ...
    assert_eq!(
        list.len(),
        4,
        "[#389] evidence count mismatch after registry churn"
    );
...
fn test_stress_evidence_preserved_after_resolution() {
    ...
    assert_eq!(
        list.len(),
        3,
        "[#389] evidence count changed after resolution"
    );
...
fn test_stress_repeated_evidence_does_not_affect_settlement() {
    ...
    for i in 0..20u32 {
        ...
        s.client()
            .submit_evidence(&tid, submitter, &cid, &SStr::from_str(&s.env, "d"));
    }
...
fn test_stress_evidence_lists_isolated_across_trades() {
    ...
    let tid_a = {
        let tid = s
            .client()
            .create_trade(&s.buyer, &s.seller, &amount, &5000u32, &5000u32, &None);
        s.client().deposit(&tid);
        s.client()
            .initiate_dispute(&tid, &s.buyer, &SStr::from_str(&s.env, "QmDisputeA"));
        tid
    };
    let tid_b = {
        let tid = s
        ...
        s.client().deposit(&tid);
        s.client()
            .initiate_dispute(&tid, &s.buyer, &SStr::from_str(&s.env, "QmDisputeB"));
        tid
    };
    ...
    s.client().submit_evidence(
        &tid_a,
        &s.buyer,
        &SStr::from_str(&s.env, "QmOnlyA"),
        &SStr::from_str(&s.env, "d"),
    );

    // Trade B must have empty evidence list
    assert_eq!(
        s.client().get_evidence_list(&tid_b).len(),
        0,
    ...
    assert!(matches!(
        s.client().get_trade(&tid_a).status,
        TradeStatus::Completed
    ));
    assert!(matches!(
        s.client().get_trade(&tid_b).status,
        TradeStatus::Completed
    ));
}
